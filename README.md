# MidTerm-Project
this is a documentation of my sanity during this project
this will be prescriped based on dates when i have touched the project and wrote something

disclaimer: if something in the code looks like it shouldn't work. i probably figured how it works. but then when the project is done.. there will be only god that knows how it works.

good luck to me! (9th nov 2025)

(9th nov 2025)
i have made the nav for the website. i want to make the Logo and the nav simple and fixed to the screen. one way to find out when its complete. but for some reason the flex does not wanna work. i wanna move the navs to the right and the logo to the left.. they are all moving together AAAAAHHH


(10th Nov 2025)

it took me almost 2 days to finally get the logo to the left and the navs to the right.. aaaa i am so proud of myself
the way? justify the flex to the right for all the nav. then calirfy margin-right for the logo to be Auto. and magically that works.. how? i have nooo ideaa.

i thought i was done with the horrors of css... now i cant make a background image for some reason... whyy whyyyyyy

(11th Nove 2025)

Finaaallyy i figured it out after hours and hours of testing.. the background image is workkingggg (well by adding it to the html style output)
but now a new problem rises... why is the webpage has a weird border? i looked at the html.. i looked at the css.. why is that happening?

okay finally fixed it.. had to makesure i set a margin for the body since for some reason the default margin was 5px.

i thought about making a video background instead of a photo.. so with learning a bit more about the video source element i had done it.. aaah i am so Proud of myself

damn.. now i don't know how to get the copy right footer to be all the way down for the webpage to look consistent
(1 hour later)
i got that done...

i want the nav to follow the scroll so its sticking to the screen.. i made a postion: fixed..
but now the background color of the body is show on the header? ha? how?
i give up on that idea ;-;

added a new section in the nav for tickets. now that i have a bit of the css styles set the rest will be easier to go through (or so i thought)

i found a nice CSS grid Generator where i can preview how the grid looks like and where i can place every class or Div..
i tried it and it made writing the code a lot easier.

